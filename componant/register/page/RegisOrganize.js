...
const RegisOrganize = ({navigation}) => {
  ...
  const {willAlert,willAsk} = useAlert();
  const {OrganizeRegister} =useAuth();
    ...
    const checkRegis = async () => {
      ...
      try{
        ...
        if(name && des && address && member?.length > 0){
            await OrganizeRegister(member,{name: name, branchname: branch,phone: phone, description: des, address: address})
            setTimeout(() => {  willAlert('ลงทะเบียนเสร็จสิ้น',''); }, 250);
            navigation.navigate('Map');
        } else {
          setTimeout(() => {  willAlert('ใส่ข้อมูลไม่ถูกต้อง','ลองตรวจสอบดูอีกครั้ง'); }, 250);
        }

      } catch (err) {

        setTimeout(() => {  willAlert('เครือข่ายมีปัญหา',err?.response?.data?.message); }, 250);
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.section}>
          ...
          <TouchableOpacity style={[styles.addressButtonContainer, {backgroundColor:'goldenrod'}]}
              onPress = {() => navigation.navigate('RegisList',{member:member,setMember: setMember})}>
