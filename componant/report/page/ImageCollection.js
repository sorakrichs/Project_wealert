...
const ImageCollection = ({navigation}) => {
    ...
    const {images,pushImage,setPage,removeImage} = useReport();
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <View style={{flexDirection:"row",alignItems:'center'}}>
                <TouchableOpacity
                    style={[styles.addImagesButton,{backgroundColor:'mediumaquamarine'}]}
                    onPress={() => {pickPicture(multiple=true).then((images) => pushImage(images))}}>
                    <Icon style={styles.iconStyle} name="folder-open" size={20}/>
                    <Text style={styles.textStyle}>ภาพจากโฟลเดอร์</Text>
                </TouchableOpacity>
                <TouchableOpacity
                    style={[styles.addImagesButton,{backgroundColor:'limegreen'}]}
                    onPress={() => {useCamera().then((image) => pushImage(image))}}>
                    <Icon style={styles.iconStyle} name="camera" size={20}/>
                    <Text style={styles.textStyle}>ภาพจากกล้อง</Text>
                </TouchableOpacity>
            </View>
                ...
                <ScrollView>
                    <View style={{flexDirection: 'row', flexWrap: 'wrap'}}>
                        {images.map((image,index) => 
                            ...
                            onPress={() => {setImageIndex(index); setImageSwipeVisible(true);}} 
                            onLongPress={
                                ()=>{

                                    willAlert('ลบรูปภาพ','จะลบรูปภาพนี้หรือไม่',() => removeImage,[index],'ask')

                                }}>
                                <Image key={index} style={styles.avatar} source={{ uri:image.path}}/>
                            </TouchableOpacity>
                        )}
                    </View>  
                 </ScrollView> :
                <View style={{alignItems:"center",justifyContent:"center",flex:1}}>
                    <Text style={{fontSize:22}}> ยังไม่มีรูปภาพ </Text>
                </View> }
